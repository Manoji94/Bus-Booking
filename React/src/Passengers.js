...
const Passengers = ({ slNo, fromLocation, toLocation, date, time, selectedSeats, routeLength }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const requests = passengerForms.map((form, index) => {
        ...
        return axios.post('http://localhost:8000/api/reserveseats/', {
          sl_no: slNo,
          date: date,
  ...
  return (
    <div className="container mt-4">
      ...
      <p>Bus Number: <strong>{slNo}</strong></p>
      <p>From Location: <strong>{fromLocation}</strong></p>
      <p>To Location: <strong>{toLocation}</strong></p>
      <p>Date: <strong>{date}</strong></p>
      <p>Time: <strong>{time}</strong></p>
      <p>Route Length: <strong>{routeLength}</strong></p>
      <p>Ticket Price Per Seat (INR): <strong>{ticketPrice}</strong></p>
      <p>Total Price (INR): <strong>{ticketPrice * passengerForms.length}</strong></p>
      <p>Selected Seats: <strong>{selectedSeats}</strong></p>
